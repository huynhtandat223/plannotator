...
| Send feedback / message | ✅ claimed by the Pi extension | ⚠️ typed via Herdr composer | ⚠️ typed via Herdr composer | ⚠️ typed via Herdr composer |
...
Feedback is delivered through one of two mechanisms, selected per agent kind by `feedbackDelivery` in the capability registry (`packages/core/live-pane-agents.ts`) — never by an agent-name check in discovery. **The two mechanisms are not equivalent, and every surface says which one is in play.**

Both mechanisms back three surfaces: response feedback, the document viewer's message composer, and the **Watch live** footer. The footer differs only in how it is addressed — it pins the pane, agent kind, session identity and advertised command list captured when the overlay opened, sends the captured `sessionId`, and is refused rather than redirected if that session has been replaced. Its user-facing copy (mechanism note, composer caveat, busy/unreadable refusal, queued-versus-confirmed receipt) is derived from the registry by `livePaneSendCopy()`, so a new agent kind is described correctly there without touching the overlay.

**Extension delivery** (`pi-extension` — Pi): loopback and Tailscale browsers may queue a batch for one live `{ paneId, sessionId }`, then only the matching local Pi extension claims it through a loopback-only endpoint and reuses Ex-Plannotator's existing `formatLiveFeedbackBatch` + `pi.sendUserMessage(..., { deliverAs: "followUp" })` delivery. A changed or closed session invalidates queued feedback. The queue uses at-most-once claim semantics to avoid duplicate prompts after crashes. This path is strictly better where it applies and remains the preferred mechanism whenever an extension is present.

**Herdr composer delivery** (`herdr-composer` — Claude Code, Codex, OpenCode): no extension exists to claim a queue entry, so the host types the (identically formatted) text into the pane's composer through Herdr and submits it with Enter (`apps/herdr-process-service/herdr-composer-delivery.ts`). The sequencing encodes hazards firstmate verified empirically (`firstmate/bin/backends/herdr.sh`): the text is typed exactly once and only Enter is ever retried (retyping duplicates the message); `/`- and `$`-prefixed messages get a longer pre-Enter settle because they open a completion popup within ~0.1s; and delivery is confirmed only by Herdr's native agent state observing a turn start — never by reading the composer back. What this path deliberately does **not** promise: it cannot verify which session is active in the pane, and it refuses to send while the agent is mid-turn (a busy agent queues typed text invisibly, so a retry would deliver twice). Responses carry `mechanism: "herdr-composer"` plus `confirmed`, refusals are `409`s with the exact reason, and the browser renders the registry's per-kind caveat instead of claiming extension-grade guarantees. An agent kind without a registry entry declares **no** delivery mechanism and stays read-only with a named reason.
