...
- **The pane/session toggle lives in the app's existing left rail** (`SidebarTabs`, `showPanesTab`/`onTogglePanes`) — captain feedback: reuse the left control strip rather than inventing a second left-edge control in the panel. The rail is `hidden lg:flex` and unmounts when the sidebar opens, so `LiveSessionTimeline` keeps its header switcher as the in-panel and mobile (sheet) trigger; `sessionListOpen`/`onSessionListOpenChange` make the disclosure optionally controlled so both drive ONE state, never two.
