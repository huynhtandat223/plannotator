...
| `PLANNOTATOR_GITBUTLER_STATUS_CACHE_MS` | How long one `but status` result is reused per (runtime, cwd) in `packages/shared/gitbutler-core.ts`. Must be a non-negative integer; invalid values use the default of `5000`. Exists so tests that mutate a fixture status and must observe the new topology can shrink the window instead of hardcoding a sleep past the default. |
...
`but status` is cached per (runtime, cwd) for `PLANNOTATOR_GITBUTLER_STATUS_CACHE_MS` (default 5s), and the picker topology revision rides in the diff `snapshotId`. Tests that mutate a status fixture and then expect a new snapshot must set that env var rather than sleeping a literal duration — hardcoded sleeps silently stop clearing the cache whenever the default TTL is retuned.
...
`apps/herdr-process-service/server.ts` can resolve a dedicated Herdr workspace by label (default `bridge-kernel-app`) for Ask AI, so the feature works without first selecting a live Pi pane. It is OFF unless a root cwd is configured via `PLANNOTATOR_ASKAI_WORKSPACE_CWD` (or `config.askAiWorkspace.cwd`); see `resolveAskAiWorkspace` in `packages/shared/config.ts`. When on, `ensureAskAiWorkspace()` resolves-or-creates the workspace (single-flight guarded) and `resolveHerdrAIWorkspace` uses it instead of throwing on a missing client cwd. Key facts: `discoverPanels()`/`panelsFromSnapshot` parse only `snapshot.agents` (every agent kind, but never a plain shell pane), so a shell-only workspace is invisible there — resolve it via `snapshot.panes` (which carry `cwd`/`foreground_cwd`/`workspace_id`) using `askAiWorkspaceFromSnapshot`. `createProcessPanel`'s workspace guard (`isKnownProcessPanelWorkspace`) accepts a caller-vouched ensured workspace id so a pi-pane-less-but-real workspace passes while unknown ids are still rejected.

### Live panes are multi-agent; capabilities are per agent kind

Herdr manages panes running Pi, Claude Code, Codex, OpenCode and whatever comes next. `panelsFromSnapshot` (`apps/herdr-process-service/server.ts`) is **agent-kind-agnostic on purpose**: it admits an entry when the snapshot supplies a pane id, a cwd and an agent kind, and never branches on the kind's name. Do not add a name check there — not even a list of the kinds we know today; a new kind must need zero edits to discovery. It carries `HerdrPanel.agent` through to `PickerMessage.agent`.

What each kind can actually do lives in **one** table: `packages/core/live-pane-agents.ts` (browser-safe, shared by the host and the editor). Identity/status/cwd/focus come from Herdr and always work. Everything richer originally came from the Pi extension's `/api/panel-session` registration, so it is declared per kind with a user-facing reason. An unlisted kind falls back to `unknownLivePaneAgentProfile`, which claims nothing.

The invariant: **a pane either works or is visibly and specifically limited** — never rich-looking and empty. Concretely: `LivePaneLimitationsNotice` (App.tsx) names each missing capability with its reason; each affordance is gated with `supportsLivePaneCapability`; every write endpoint refuses via `refusedUnsupportedCapability` with the same reason and a `409`. Claude Code panes get a real read-only transcript by reusing the existing cwd-keyed reader `apps/hook/server/session-log.ts` (adapter: `apps/herdr-process-service/claude-live-session.ts`) — do not write a second transcript parser. Sends have TWO mechanisms selected per kind by `feedbackDelivery` in the registry: Pi keeps the extension claim queue (strictly better, always preferred where an extension exists); Claude Code/Codex/OpenCode use Herdr composer delivery (`apps/herdr-process-service/herdr-composer-delivery.ts` — type once, Enter-only retries, native agent-state turn-start confirmation, refuse while busy; hazards from `firstmate/bin/backends/herdr.sh`), with the weaker guarantees stated verbatim via each kind's `composerDeliveryCaveat`. Never claim parity between the two in UI copy. Full per-kind matrix: `apps/herdr-process-service/README.md`.

### Ex-Plannotator live pane: chat transcript, activity trail, handoff UI

The live pane (`liveMessageReview`) renders as a two-sided chat transcript, not a single-message picker. Key seams (all additive/optional so non-live surfaces are untouched):
- **Transcript** is the `MessagesBrowser` list itself, so it inherits the existing per-pane retention (`LIVE_MESSAGE_RETENTION` = 20), `+N more` paging, scroll anchor, and Jump-to-latest. `chatLayout` (bubbles) turns it into a transcript; echo anchoring is keyed by `messageId` so order is irrelevant. Agent rows stay `<button>` annotation targets; `CaptainEchoRow` (right-aligned `bubble` variant) and activity rows are never targets. Do NOT add a `role` to the wire, relax the assistant-only capture filter (`apps/ex-pi-extension/assistant-message.ts`), or make the queue claim non-destructive.
- **The transcript is NEWEST-FIRST, in real DOM order** (captain rejection of #47's oldest-first chat log). The wire is already newest-first, so `stableLiveTimelineMessages` passes it straight through and the panel omits `chronological` — reading order, tab order and the assistive reading all agree, which a CSS reversal would break. `chronological` remains a supported `MessagesBrowser` mode and stays covered; just do not reintroduce it here or re-add a `.reverse()` anywhere on the live path.
- **Panel space and opening scroll are a contract, not styling.** The pane list is a toggle, not a permanent band — closed, the response history owns the panel. Newest-first means the latest turn is the TOP, so `pinLatestKey` (keyed on **session identity, not `messages`**) lands opening/switching at `scrollTop 0` while an arriving frame never moves the viewport. Verify that by measuring drift of a text-keyed row across live frames — not by watching `scrollTop`, which paging and the anchor both change legitimately.
- **Scroll anchoring is element-based, and must stay that way.** The row budget makes this a WINDOWED list: a prepended live turn also evicts one off the far end, so total height barely changes while every visible row slides down a slot — height arithmetic alone drifted a measured 82px. `MessagesBrowser` anchors on a surviving `[data-message-row]` (`anchoredScrollTopForRowShift`), falling back to `anchoredScrollTopForGrowth` only when that row was evicted. Two exemptions are deliberate: a reader at `scrollTop 0` is *following the latest* and is never moved by a live frame, and `+N more` in a newest-first list appends BELOW the viewport (`page-append`), where any compensation would itself be the bug.
- **What the left rail toggles is the WHOLE panel, not a control inside it.** Captain feedback rejected moving the pane switcher onto the rail (#48): the thing they want back is the space. `agentResponsePanelWrapperClass` (`packages/editor/live/agentResponsePanelLayout.ts`) takes the block out of the flex flow (`absolute` + `invisible`, plus `aria-hidden`/`inert`) so the document reclaims the entire area, while the panel stays MOUNTED — scroll offset, `+N more` paging and the live anchor exist only in the DOM, so unmounting would silently discard them. Keep the hidden box the same size as the shown one for the same reason. The pane switcher stays in the panel header where #47 put it.
- **That toggle has three homes because the rail has breakpoints, and all three drive one state** (`liveResponsePanelVisible` in App.tsx). `SidebarTabs` and `SidebarContainer`'s tab bar are both `hidden lg:flex` — the rail also unmounts when the sidebar opens — so below `lg` the `AppHeader` cluster carries it. Label/glyph/class come from `packages/ui/components/agentResponsePanelToggle.tsx` so they cannot drift. A panel that can be hidden must be recoverable at every width: never add a hide affordance without checking which surface owns the un-hide at that breakpoint.
- **That per-mode check is now an enforced invariant, because enumerating modes by hand missed one.** Wide/focus mode unmounts the rail AND closes the sidebar while leaving the live panel painted, so at ≥`lg` the panel sat on screen with no control at all (browser-measured: 380×558 panel, zero toggles with a non-zero box). The homes and the panel's on-screen condition are pure functions in `packages/editor/live/agentResponsePanelLayout.ts` (`sidebarRailMounted`, `agentResponsePanelOnScreen`, `agentResponseToggleHomes`, `agentResponseToggleReachable`), and a test sweeps EVERY combination of layout flags × breakpoint asserting `panel on screen ⇒ toggle reachable`. Two consequences to preserve: the rail outlives wide/focus mode during live review carrying only that one flag (`showTocTab={false}` and friends keep wide mode's "panels away" promise), and the header copy drops its `lg:hidden` only when it is the last home standing. Add a layout mode → the sweep tells you whether you stranded the panel; do not answer it by hand.
- **`railMounted` in App.tsx is one source of truth for the rail and the document's `lg:pl-[30px]` gutter.** They were near-duplicate conditions; if they drift the rail overlaps the text it sits beside.
- **A pane has more than one plausible cwd, and `foreground_cwd` is not always the right one.** `panelsFromSnapshot` still prefers Herdr's `foreground_cwd` for `HerdrPanel.cwd` — usually the most specific answer, e.g. the git worktree an agent is really in rather than the checkout its shell started in. But it follows the FOREGROUND PROCESS, so an agent running a language server reports the SERVER's directory: an observed Claude pane in `.../projects/dms-workspaces` reported `~/.local/lib/node_modules/pyright/dist` and, being keyed on that, silently lost its entire transcript while Watch showed it plainly alive. The agent's own cwd is therefore carried as `HerdrPanel.agentCwd` (only when it differs) and used as a FALLBACK by `readClaudeLiveSessionForCwds` — foreground first, so the worktree case does not regress into showing the parent checkout's session. Two consequences: anything else that resolves a pane to something cwd-keyed should take the same pair, and `diskTranscriptOutcomes` now runs its shared-directory ambiguity check on the RESOLVED cwd rather than the pane's, because two panes can fall back onto one directory while looking unique beforehand — that is a collision only resolution can see.
- **Activity trail** (`read → grep ×3 → edit → bash`, names only, no payloads): the extension appends an ordered, bounded `activityTrail` (`HERDR_ACTIVITY_TRAIL_LIMIT`, consecutive repeats collapsed to `count`) in `apps/ex-pi-extension/herdr-registration.ts`, reset per turn via `resetHerdrActivityTrail` on `agent_start`. It rides the existing `/api/panel-session` frame → `PickerMessage.activityTrail`. Render helpers in `packages/editor/liveActivityTrail.ts`; the instantaneous chip stays in `liveActivityChip.ts`. Inline renders show the LATEST step only (captain feedback: the full chain was too verbose); the full arrow-joined chain is behind an on-demand expand.
- **Pinned pane chips** (header switcher, one chip per live pane): pure derivation (identity `workspace · tab`, active-first ordering, `+N more` overflow, CTX warning at ≥75%, recent commands) in `packages/editor/livePaneChips.ts`; presentational row in `packages/editor/LivePaneChipsRow.tsx`. Clicking a chip drives the SAME `handleSelectMessage` selection the annotate picker drives — chips own "which pane am I looking at", the picker still owns "which message to annotate". The tab name is surfaced as first-class `paneTab` (server `reviewSnapshotFromPanels`, `PickerMessage.paneTab`) so same-workspace panes stay distinguishable. Part 2 recent commands: bash-tool `activityTrail` entries carry a redacted+truncated `command` summary built in `apps/ex-pi-extension/commandSummary.ts` (redaction BEFORE truncation; allowlist of bash-like tools; never logs/returns a raw command) — latest inline, older behind an expand.
- **Ex AI Companion handoff** (`packages/editor/components/ExAIChatPanel.tsx`, `useExAIChat.ts`, coordinator `apps/herdr-process-service/ex-ai-companion.ts`): panel shows an always-visible status header (`exAIStatusBadge` maps `ExAICompanionState.status`) and one-click handoff of suggested replies (reserve-before-claim, idempotent per `suggest:<boundary>:<index>`). Eligibility (`exAIEligible = exAIIdentity !== null`, App.tsx ~4050) is correct-as-designed: it requires an in-review live message carrying both `paneId` and `piSessionId` and not being a companion pane. `exAIIdentity` is memoized on primitives to keep object identity stable — do NOT return a fresh object literal per render (it reintroduces the ~88 req/s refresh loop).
- **Context-high handoff WARNING** (`apps/herdr-process-service/context-handoff.ts` pure detector + `evaluateContextHandoffs` in `server.ts`; UI `LivePaneHandoffWarning` in `packages/editor/App.tsx`): a per-pane Schmitt-trigger + debounce state machine watches every live pane's `contextUsage.percent` on the existing 2s snapshot refresh (NO new poll loop) and publishes an optional `contextHandoff` field into the snapshot message. It only WARNS — the captain fires the handoff MANUALLY via the button, which reuses `POST /api/command` → the pane's registered `handoff-to-continue`-class Pi command (button disabled with a legible reason when the pane registers none). Hysteresis: arm+warn at HIGH (72%, `PLANNOTATOR_HANDOFF_HIGH_PERCENT`), re-arm only below LOW (55%, `PLANNOTATOR_HANDOFF_LOW_PERCENT`), debounce `PLANNOTATOR_HANDOFF_DEBOUNCE_TICKS` (2); `percent === null` is no-signal (post-compaction re-arm). Never coerce `null`→0/100, never auto-send.

### Watch live: Herdr terminal observation, plus a message composer that is not part of it

**Watch live** shows one explicitly selected pane's current visible terminal screen in a full-screen mobile-first overlay, with a fixed footer that sends ordinary text to that same pane. The terminal half is an *observation* surface and the observe/control boundary is the whole point — so the design puts that boundary where it cannot be crossed by accident rather than relying on discipline:

- **No screen-change subscription exists.** On the supported release (herdr 0.7.3, protocol 16) `events.subscribe` accepts exactly `pane.output_matched`, `pane.agent_status_changed`, `pane.scroll_changed`; `pane_output_changed` (with its `revision`) lives only on an unfiltered client feed no CLI exposes. So `apps/herdr-process-service/pane-watch.ts` polls `herdr pane read <pane> --source visible --format ansi` — but **only while a viewer is attached**, revision-gated so an unchanged screen sends nothing. Do not "fix" this into an always-on capture loop, and re-check the schema before assuming a subscription is now available.
- **The browser sends only an opaque pane id.** Source, format and every other argument are fixed host-side (`packages/core/pane-watch.ts` states them once). The id is authorized against a *fresh* `discoverPanels()`; ongoing liveness reuses the existing `liveSnapshotPublisher` — never a second poll loop.
- **Frames never enter `HerdrLiveState`, `PickerMessage`, or `/api/session/events`.** `GET /api/pane-watch` is a dedicated per-viewer SSE stream; putting terminal content on the shared snapshot would broadcast a watched screen to every connected browser.
- **Not a terminal emulator.** `packages/core/ansi-screen.ts` is an SGR-only parser producing styled runs, deliberately not xterm/WebTUI: an emulator carries a cursor, a keyboard handler and a write side, and the annotate PTY transport (`packages/server/agent-terminal.ts`) creates and controls a process. The pane-watch transport still has no write handle at all.
- **The composer footer writes MESSAGES, never keystrokes, and that is a structural claim.** `LivePaneWatchOverlay` has one textarea + **Send message** (⌘/Ctrl+Enter; plain Enter is left to the browser so prose keeps its paragraphs). It posts one complete message to the existing `/api/instruction`, so `pane-watch.ts` stays observation-only and never grows a send/keys/focus/resize verb. The screen region holds no control, no focusable element and no `tabindex`; the footer disappears once the watch has `ended`. Advertised commands are a SEPARATE control with their own **Run /name** action on `/api/command` — there is no autocomplete in the textarea, so nothing the captain types can execute itself. That control also REQUIRES the captured session (`watchCommandBlockedReason`): omitting `sessionId` is how a caller opts INTO the unpinned "resolve the current registration" path, so a missing capture must disable the action visibly rather than silently become it.
- **Composer delivery refuses text that starts with `/` or `$`, and that refusal is not a style choice.** `deliverViaHerdrComposer` types once then presses Enter up to three times, because a popup-swallowed first Enter starts no turn. Sending `/model` to a real Claude Code pane therefore walked popup → submit → **interactive picker confirm** on three blind Enters, changing a setting the captain never saw. The loop cannot tell a consumed keystroke from an ignored one, so no retry count is safe, and sending a single Enter is worse (it leaves the command armed in a composer Watch cannot clear). The refusal is the FIRST statement in the delivery function — before the agent-state read — so no call site can opt in, and `composerCommandTextRefusal` in `packages/core/live-pane-agents.ts` lets the browser refuse in the same words without a request. It keys on the FIRST character only: paths, dates and URLs in ordinary prose must still send. Extension delivery is untouched (it presses no keys). The old `popupSettleMs` is gone with the branch it timed. Keystroke forwarding (issue #57) is the real answer — there the captain's own Enter is the only Enter.
- **The target is pinned as one object at open, and only `agentStatus` is refreshed.** `watchTarget` in `App.tsx` captures `{ paneId, label, agent, piSessionId, commands }` from the message selected at that instant; the send handlers read that object and never `selectedLiveMessage`. Status is re-read BY the pinned pane id (a busy refusal has to be about now); a differing current `piSessionId` sets `sessionReplaced`, which refuses both writes visibly rather than addressing the replacement. Pi sends carry the captured `sessionId` and `instructionSessionRefusal()` 409s a replacement instead of queueing to it; `commandDelivery()` takes the same session optionally, so the palette/handoff callers that resolve at click time are unchanged.
- **Delivery copy comes from `packages/core/live-pane-agents.ts` (`livePaneSendCopy`), never from an agent name**, and the two mechanisms are never described as equal: an extension send is **queued** (the claim is out of band and never reported back — do not upgrade this wording without an end-to-end claim receipt), a composer send is typed and either turn-confirmed or explicitly unconfirmed, and an unconfirmed one offers NO retry because retyping is how a message gets delivered twice. `COMPOSER_BUSY_REASON`/`COMPOSER_UNREADABLE_REASON` moved to that registry so the browser can refuse before a request in the host's exact words. A kind with no mechanism gets no textarea at all — read-only means nothing to type into, plus the registry's own reason.
- **Watch has TWO write paths, and they are opposites on purpose — never merge them.** `Send message` (`/api/instruction`) delivers one complete message and reports a receipt, but must press Enter to submit, which is why it refuses `/`-prefixed text (bullet above). `Type into pane` + the key row (`POST /api/pane-input`, `packages/core/pane-input.ts` + `apps/herdr-process-service/pane-input.ts`) is the exact inverse: it puts characters in the agent's OWN composer and presses **nothing**, so the agent's native completion popup opens in the screen the captain is already watching and they drive it with discrete key presses. That is the only way `/model` works, and it is why the copy never calls it delivery — there is no busy gate, no confirmation, no retry, and the screen is the acknowledgement. Four constraints hold it in place: the browser sends a pane id plus a key from a CLOSED enum (`PANE_INPUT_KEYS`; the host owns the Herdr key names, so no modifier chords, function keys or pane lifecycle are reachable); `text` refuses control characters, because a newline smuggled in as text would be a submit nobody pressed; the pane is re-authorized against a FRESH `discoverPanels()` on every call; and calls are serialised **per pane** (`paneInputQueue`) because `text` then `key` is the whole interaction and two overlapping `execFile`s would let Tab land in an empty composer with nothing looking wrong. Raw typing is pane-scoped, not agent-kind-scoped — an agent kind the registry has never heard of still gets it, which is the point.
- **A Watch message is not feedback.** The handler sends only the composer's text and touches no annotation/`getNonGlobalFeedbackPayload` state, so it can never become a review submission the way the compatibility viewer path deliberately does. `packages/editor/watchComposerWiring.test.ts` pins that, the pinning, and the session rules against `App.tsx` source — the only reachable way to assert this half without mounting the app.
- **The overlay's DOM test renders the composer in every case, on purpose.** The previous "no textarea anywhere / only Close" assertions would have stayed green against an overlay mounted without the callbacks — passing while proving nothing about the feature that invalidated them. Assert the boundary you still hold (no control *inside* the screen region), not the absence of the feature.
- **Reconnect clears the screen.** `reconnecting` removes the rendered frame before the gap — a stale terminal presented as live is worse than none. A pane Herdr stops reporting is terminal (`Session ended`).
- **`h-screen h-[100dvh]` is NOT a fallback pair — it is `100vh`.** Both are `height` utilities at equal specificity in one `@layer utilities`, so emission order decides, and Tailwind v4 emits `.h-screen{height:100vh}` AFTER `.h-\[100dvh\]{height:100dvh}` (verify in a built bundle's `<style>`, not by reading the class attribute). On a `fixed` overlay that is not cosmetic: `100vh` is the LARGE viewport, so the box ran ~56px past the visible one on Android Chrome and the bottom of the write surface rendered off the device edge — unreachable, because nothing scrolls a fixed element. The overlay now declares its height ONCE (`h-[100dvh]`, with `inset-0` as the no-`dvh` fallback), and its DOM test asserts *exactly one* height utility rather than the presence of both — the old test required both and so ratified the bug. **`packages/editor/App.tsx`'s document root still carries the same pair**; fixing it changes the whole app's mobile height, so it needs its own verification pass.
- **A `fixed` overlay has no scroll of last resort, so every region inside it must be bounded or shrinkable.** The Watch composer is `flex-shrink-0` (the terminal is what gives up space), which protected it from being squeezed but let it grow off the bottom edge: measured, its controls left the viewport below ~381px of height with `document.scrollingElement` unscrollable. It is now additionally `max-h-[60%] overflow-y-auto`. The cap is loose on purpose — real phone portraits measure 306–368px of composer against 640–848px of viewport, so it never binds there and phone/desktop layout is unchanged; it binds only on genuinely short viewports, where it also leaves the terminal a ~40% floor.
- **Two environment facts worth not rediscovering:** happy-dom silently DROPS `100dvh` from inline styles (so the dynamic viewport is stated as a class and the DOM test asserts the class — it has no layout engine, so anything about real geometry has to be measured in a browser); and `herdr workspace close` reassigns focus even for a `--no-focus` workspace that was never focused, so the integration test's fleet tripwire covers workspace/tab/pane ids but not focus.
- **Cleanup must be armed before the first `await`.** Opening a watch spends several hundred ms in authorization and the first screen read; `close` fires once, so a listener attached after those awaits never runs, and a disconnect in that window leaks the timer, heartbeat, liveness subscription and capacity slot — the poll keeps reading the pane forever, which is the one thing this feature must never do. `startPaneWatchStream` therefore arms an idempotent `stop()` first and takes the capacity slot under a `counted` flag.
- **That leak has no reliable external symptom, which is why it has its own test.** Orphaned reads are masked (the first `response.write` to a dead socket returns false → `draining` latches, and `drain` never fires on a dead socket, so the loop lives but stops reading); and 32 spread-out aborts failed to exhaust capacity because `close` *usually* lands after the handler tail. A test that passes only because a race normally resolves safely is not evidence, so `pane-watch.early-disconnect.test.ts` forces the ordering with a fake request/response — a deliberate, documented exception to "do not unit-test the transport". Verified red without the fix, green with it.
- **The terminal font floor is a legibility floor, not a fitting parameter.** Real panes are ~215 columns; a 390px phone wants ~3px to fit one, so the clamp always bottoms out on mobile. Shipped at 7px it rendered as an all-but-blank black screen that still needed 2.3x horizontal scrolling — and once you are scrolling anyway, shrinking below readable buys nothing. `MIN_FONT_PX` is 11 and may only be tuned against readability. The local browser check *measured* 7px and passed it as "the fit rule works": assert the outcome (a legible size), never just the mechanism.
- **Verifying the deployed URL from inside WSL does not work, and that is not an outage.** `100.70.216.87` is mirrored onto `eth0` by WSL2 mirrored networking (no `tailscale0` device; the in-WSL `tailscale` CLI is a separate, logged-out daemon and is a red herring). WSL cannot hairpin to the host's own tailnet address, so curl/Chrome hang there while the same listener answers 200 on `127.0.0.1` and the LAN IP. Confirm the tailnet with the host binary at `/mnt/c/Program Files/Tailscale/tailscale.exe` before concluding anything is down.
- **`pkill -f <pattern>` matches your own shell.** `pkill -f "herdr-process-service/server.ts"` kills the bash process running it (exit 144) and can match unrelated agents whose command line quotes the pattern. Resolve PIDs and filter by `/proc/<pid>/cwd` instead — the production service is the systemd unit `plannotator-herdr.service`, running from the primary checkout.
- **Reserve the capacity slot in the same tick as the check.** `activeWatches` is global mutable state; a check-then-`await`-then-increment lets every concurrent open pass the gate at zero, so the bound only ever constrained opens arriving strictly one after another. Test and reservation must stay adjacent with no `await` between them.
- **Two assertion traps in a LIVE Herdr session — do not reintroduce either.** (1) Never assert Herdr's focused pane: the integration test drives output with `herdr pane run`, which focuses the pane it runs in, so it measures the harness, not Watch. (2) Never fingerprint the whole fleet before/after: other agents open and close panes continuously, so it fails for their activity. Isolation is instead *enforced* — `realHerdr` refuses any command naming a resource the test did not create (the id regex needs the digit in `w\d…`, or it also matches the literal word `workspace`) — plus an end assertion that nothing it created survives. Both halves are immune to unrelated activity; the old sampling version flaked about one run in three.
- **Real coverage:** `apps/herdr-process-service/pane-watch.integration.test.ts` (gated behind `PLANNOTATOR_HERDR_WATCH_E2E=1`) creates its own workspace, gives the pane an agent kind Herdr has never seen, and proves the properties externally via a logging `herdr` shim on the spawned server's PATH — no reads before a viewer, reads stop on disconnect, and no write verb ever issued.
...
**Important: Tailwind `@source` paths.** When creating new directories that contain `.tsx` files with Tailwind classes, add a matching `@source` entry to the app's `index.css`. Tailwind only generates CSS for classes it finds in scanned files — missing paths means classes appear in the DOM but have no effect. **Extension matters as much as directory:** the globs are per-extension, so moving class strings out of a `.tsx` into a plain `.ts` helper in an already-listed directory drops them too. The failure is silent and only visible in a browser — the class is on the element and does nothing.
...
**A restart is not a build.** `apps/herdr-process-service/server.ts` serves that prebuilt bundle verbatim and never consults the source tree, so merging UI work and restarting the service looks exactly like a deploy and changes nothing — the browser keeps rendering the previous UI. That is how a shipped left-rail control came to be "missing" from a live instance whose source was three merges ahead of the HTML it was serving, with every unit test passing because the code under test was never the code on screen. The service now compares mtimes at startup and prints an actionable warning (`apps/herdr-process-service/editor-bundle-freshness.ts`); when verifying UI in a browser, confirm the bundle is fresh before believing anything you see, and grep the served HTML for a string only the new code contains.
