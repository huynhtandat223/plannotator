...
### Watch live: read-only Herdr terminal observation

**Watch live** shows one explicitly selected pane's current visible terminal screen in a full-screen mobile-first overlay. It is an *observation* surface, and the observe/control boundary is the whole point — so the design puts that boundary where it cannot be crossed by accident rather than relying on discipline:
...
- **Not a terminal emulator.** `packages/core/ansi-screen.ts` is an SGR-only parser producing styled runs, deliberately not xterm/WebTUI: an emulator carries a cursor, a keyboard handler and a write side, and the annotate PTY transport (`packages/server/agent-terminal.ts`) creates and controls a process. Watch has no write handle because there is nothing to write to.
