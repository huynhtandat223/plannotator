...
- **Transcript** is the `MessagesBrowser` list itself, so it inherits the existing per-pane retention (`LIVE_MESSAGE_RETENTION` = 20), `+N more` paging, scroll anchor, and Jump-to-latest. `chronological` (oldest-first) + `chatLayout` (bubbles) props turn it into a transcript; the chronological budget keeps the NEWEST rows (tail). App passes a reversed copy (`sidebarMessages`) — echo anchoring is keyed by `messageId` so order is irrelevant. Agent rows stay `<button>` annotation targets; `CaptainEchoRow` (right-aligned `bubble` variant) and activity rows are never targets. Do NOT add a `role` to the wire, relax the assistant-only capture filter (`apps/ex-pi-extension/assistant-message.ts`), or make the queue claim non-destructive.
- **Panel space and opening scroll are a contract, not styling.** In `LiveSessionTimeline` the session switcher is capped (`max-h-36`) well under the transcript's `flex-1` share — the response is what the column is for, and the switcher once occupied more of the panel than the response it switches between. A chronological transcript's newest turn sits at the BOTTOM, so a fresh mount at `scrollTop 0` shows the OLDEST turn; `MessagesBrowser`'s `pinLatestKey` fixes that and is keyed on **session identity, not `messages`**, so opening or switching lands on the latest while an arriving frame never moves the viewport. Verify that invariant by measuring drift of a text-keyed row across live frames — not by watching `scrollTop`, which paging and the growth anchor both change legitimately.
