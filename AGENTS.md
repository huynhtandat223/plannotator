...
- **Do not assert Herdr's focused pane in these tests.** The integration test drives output with `herdr pane run`, which focuses the pane it runs in, so a focus assertion measures the harness rather than Watch — it flaked exactly that way. "Watch never changes focus or size" is proved by the absence of any focus/resize verb in the server's own Herdr invocation log.
